...
public class Jardin extends Applet {
    ...
    volatile static public boolean gateteLogico = false;
    //    Checkbox fixit;
    ...
    private void go() {
        ...
        torno1= new Torno(torno1D,contador);
        torno2= new Torno(torno2D,contador);
        torno1.start();
        torno2.start();
    }
...
class Contador {
    ...
    void incrementa() {
        ...
        CC.FuerzaCC();
        while(!Jardin.gateteLogico) {}
        value=temp+1;       //write[v+1]
        Jardin.gateteLogico = false;
        display.setvalue(value);
...
class Torno extends Thread {
  ...
  Torno(NumberCanvas n,Contador c)
    { display = n; people = c; }
